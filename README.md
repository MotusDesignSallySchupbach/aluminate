...
The firmware development environment is based on a [devcontainer](https://code.visualstudio.com/docs/devcontainers/containers) model ([specification](https://containers.dev/)).
...
1. Install [Git for Windows](https://github.com/git-for-windows/git/releases/latest) in order to ensure [Git Credentail Manager (GCM)](https://learn.microsoft.com/en-us/windows/wsl/tutorials/wsl-git#git-credential-manager-setup) is properly installed and setup.
...
* Open a terminal for your WSL Ubuntu image using the Windows Teminal program
* Install github cli tool and setup git. This in conjunction with the Git Credential Manager setup done earlier will allow you to use these credentials in your containers as well.
    * `sudo apt update`
    * `sudo apt install gh`
    ...
    * References: [Git Installation](https://learn.microsoft.com/en-us/windows/wsl/tutorials/wsl-git), [Git Config](https://learn.microsoft.com/en-us/windows/wsl/tutorials/wsl-git#git-config-file-setup)
...
* NOTE: If not already running, start Docker Desktop. It should always be running in the background when using a devcontainer
...
NOTE: These instructions assume `usbipd-win 4.0.0` or higher installed

1. Install [USBIPD-WIN](https://learn.microsoft.com/en-us/windows/wsl/connect-usb) in Windows
1. Install ST Link tools in WSL instance. This will ensure the proper udev rules are applied for ST USB debuggers. Alternatively, the STM32CubeCLT from the next step can used in this step.
...
NOTE: If you prefer J-Link, you can install the J-Link debian package and setup a J-Link debugger in the `launch.json` file.

## Compile and run unit tests locally (posix build)
In a VSCode container bash terminal, run the following:
* `west unit`

This runs a custom west script that calls the Zephyr test tool, `twister`, to run all app unit tests including code coverage results.
...
You can provide you own formatting by replacing the `.clang-format` file in the `.vscode` folder.
...
You can create your own templates by editing the `snippets.code-snippets` file in the `.vscode` folder.
